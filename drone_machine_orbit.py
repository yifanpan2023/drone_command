...
def rotate(period):
    the_connection.mav.send(mavutil.mavlink.MAVLink_set_position_target_local_ned_message(10, the_connection.target_system,
							...
							int(0b011111111000), 0, 0, -DEAFAULT_ALT, 0, 0, 0, 0, 0, 0, 0, 2*math.pi/period))
...
while (1):
	...
	current_timestamp = int(message_received[80:])
	current_command = message_received[:80]
		...
		if (message_received == prev_message):
			...
			if(current_command != prev_command):
				...
				#14
					...
					if ((msg.lat-int(target_lat[orbit_index]))**2 +(msg.lat-int(target_lat[orbit_index]))**2 <300):
				...
				#15
					...
					if (command_type == 3):	
						orbit_index = int(message_received[76:80])
						go_pos(int(target_lat[orbit_index]), int(target_lon[orbit_index]), target_rad[orbit_index])
						orbit_state = WAIT_MOVEMENT
				#10
